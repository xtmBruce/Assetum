...
const NotificationsPage: React.FC = () => {
  ...
  return (
    <div className="p-4 md:p-8 max-w-[1000px] mx-auto animate-in fade-in duration-300">
      <div className="mb-8">
        <h1 className="text-2xl md:text-3xl font-black tracking-tight text-[#111318] dark:text-white">Operational Activity Hub</h1>
        <p className="text-[#616f89] text-sm md:text-base font-medium">Real-time alerts, maintenance requirements, and lease triggers.</p>
      </div>

      <div className="bg-white dark:bg-gray-900 border border-[#f0f2f4] dark:border-gray-800 rounded-xl overflow-hidden shadow-sm">
        <div className="p-4 border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/30 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex gap-4 overflow-x-auto">
             {['All', 'Unread', 'Urgent'].map(f => (
               <button 
                key={f}
                onClick={() => setActiveFilter(f)}
                className={`text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeFilter === f ? 'text-primary' : 'text-[#616f89] hover:text-primary'}`}
               >{f}</button>
             ))}
          </div>
        ...
        <div className="divide-y divide-gray-100 dark:divide-gray-800">
          {filteredNotifications.length === 0 ? (
            ...
            filteredNotifications.map((n) => (
              <div key={n.id} className={`p-4 md:p-6 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors flex gap-4 ${n.urgent && !n.read ? 'border-l-4 border-l-red-500' : ''}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start mb-1 gap-2">
                    ...
                    <span className="text-[10px] font-bold text-[#616f89] uppercase whitespace-nowrap">{n.time}</span>
                  </div>

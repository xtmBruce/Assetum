...
const UserAdminPage: React.FC = () => {
  ...
  const handleDelete = (id: string) => {
    const user = users.find(u => u.id === id);
    if (!user) return;
    if (window.confirm(`Remove ${user.name} from the system?\n\nThis will revoke all access permissions immediately.`)) {
      deleteUser(id);
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col p-4 md:p-8 animate-in fade-in duration-300">
      <div className="flex flex-wrap justify-between items-end gap-4 mb-10">
        <div>
          <h1 className="text-2xl md:text-3xl font-black tracking-tight text-slate-900 dark:text-white">User Administration</h1>
          <p className="text-slate-500 max-w-lg mt-1 font-medium text-sm md:text-base">System-wide access control, role provisioning, and account audits.</p>
        </div>
      ...
      <div className="flex flex-wrap gap-4 items-center justify-between mb-6">
        <div className="flex gap-2 bg-slate-100 dark:bg-gray-800 p-1 rounded-xl shadow-inner overflow-x-auto">
          {['All Users', 'Admins', 'Inactive'].map((tab) => (
            <button 
              key={tab} 
              onClick={() => setActiveTab(tab)}
              className={`px-4 md:px-5 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === tab ? 'bg-white dark:bg-gray-700 text-primary shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}
            >
              {tab}
            </button>
          ))}
        ...
        <div className="relative w-full md:w-auto">
          ...
          <input 
            ...
            className="w-full md:w-80 border-slate-200 dark:border-gray-800 rounded-xl pl-10 py-2.5 text-xs dark:bg-gray-900 focus:ring-primary shadow-sm transition-all" 
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-50/50 dark:bg-gray-800/30">
              <tr>
                <th className="px-4 md:px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Team Member</th>
                <th className="px-4 md:px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Security Role</th>
                <th className="px-4 md:px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Status</th>
                <th className="px-4 md:px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Actions</th>
              </tr>
            </thead>
                  ...
                  <tr key={user.id} className="hover:bg-slate-50/50 dark:hover:bg-gray-800/20 transition-colors">
                    ...
                    <td className="px-4 md:px-6 py-4 text-right">
                      <div className="flex justify-end items-center gap-4">
                        <button onClick={() => handleResetPassword(user.email)} className="text-[9px] font-black uppercase tracking-widest text-slate-500 hover:text-primary transition-colors">Reset Pwd</button>

...
const AddAssetPage: React.FC = () => {
  ...
  return (
    <div className="max-w-[1000px] mx-auto py-4 md:py-8 px-4 md:px-8 animate-in fade-in duration-300">
      <div className="mb-8">
        ...
        <h1 className="text-2xl md:text-3xl font-black tracking-tight text-[#111318] dark:text-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in slide-in-from-bottom-2">
          {assetTypes.map((t) => (
            <button
              ...
              className="group p-6 md:p-8 bg-white dark:bg-gray-900 border-2 border-[#f0f2f4] dark:border-gray-800 rounded-2xl text-left hover:border-primary transition-all flex items-center gap-4 md:gap-6 shadow-sm"
            >
              <div className="size-12 md:size-16 rounded-xl bg-primary/10 flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-white transition-colors">
                <span className="material-symbols-outlined !text-3xl md:!text-4xl">{t.icon}</span>
              </div>
              <div className="flex-1">
                <h3 className="text-lg md:text-xl font-bold mb-1 text-[#111318] dark:text-white">{t.label}</h3>
                <p className="text-[#616f89] text-xs md:text-sm">Automated tracking templates for {t.id.toLowerCase()}.</p>
              </div>
            </button>
          ))}
        </div>
      ...
      {step === 2 && (
        <form onSubmit={handleSubmit} className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
          <section className="bg-white dark:bg-gray-900 rounded-xl border border-[#dbdfe6] dark:border-gray-800 p-6 md:p-8 shadow-sm">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-sm font-semibold">Asset Image</label>
                    <div className="flex gap-4 items-start">
                      <div className="flex-1">
                        <input 
                          ...
                          className="w-full rounded-lg border-gray-300 dark:bg-gray-800 dark:border-gray-700 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-primary file:text-white hover:file:bg-primary-hover file:cursor-pointer" 
                        />
                        <p className="text-[10px] text-slate-400 mt-1">Upload an image or leave empty for default placeholder</p>
                      </div>
                      <div className="size-16 rounded-lg bg-slate-100 dark:bg-gray-800 border-2 border-white dark:border-gray-700 shadow-sm shrink-0 overflow-hidden">
                        {formData.img ? (
            ...
            <div className="mt-8 pt-8 border-t dark:border-gray-800 flex flex-col sm:flex-row justify-between items-center gap-4">
              {!id && (
              ...
              <div className="flex gap-3 ml-auto w-full sm:w-auto">
                <Link to="/inventory" className="flex-1 sm:flex-none px-6 py-2.5 rounded-lg border dark:border-gray-700 font-bold text-sm text-center">Cancel</Link>
                <button type="submit" className="flex-1 sm:flex-none px-10 py-2.5 bg-primary text-white rounded-lg font-bold text-sm shadow-lg hover:bg-primary-hover active:scale-95 transition-all">

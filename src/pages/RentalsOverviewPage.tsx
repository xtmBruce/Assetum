...
const RentalsOverviewPage: React.FC = () => {
  ...
  return (
    <div className="p-4 md:p-8 max-w-[1400px] mx-auto animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-black tracking-tight text-slate-900 dark:text-white">Fleet Rentals & Leases</h1>
          <p className="text-slate-500 mt-1 font-medium text-sm md:text-base">Lifecycle management for active agreements and pending asset returns.</p>
        </div>
        <Link 
          to="/rentals/new" 
      ...
      <div className="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-xl overflow-hidden shadow-sm">
        <div className="p-4 md:p-6 border-b border-slate-100 dark:border-gray-800 flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-slate-50/20 dark:bg-gray-800/20">
          <div className="flex gap-2 p-1 bg-slate-100 dark:bg-gray-800 rounded-lg overflow-x-auto">
            {['All', 'Active', 'Overdue', 'Completed', 'Terminated'].map((tab) => (
              <button 
                ...
                className={`px-3 md:px-4 py-1.5 rounded-md text-[10px] font-black uppercase tracking-wider transition-all whitespace-nowrap ${
                  activeTab === tab 
          ...
          <div className="relative">
            ...
            <input 
              ...
              className="pl-10 pr-4 py-2 bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-700 rounded-lg text-xs w-full md:w-80 focus:ring-primary transition-all"
            />
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-50 dark:bg-gray-800/50">
              <tr>
                <th className="px-4 md:px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Lease Ref</th>
                <th className="px-4 md:px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Client & Asset</th>
                <th className="px-4 md:px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Contract Value</th>
                <th className="px-4 md:px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right hidden md:table-cell">Settled</th>
                <th className="px-4 md:px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Status</th>
                ...
                filteredRentals.map((rental) => {
                  return (
                    <tr key={rental.id} className="hover:bg-slate-50/50 dark:hover:bg-gray-800/30 transition-colors">
                      <td className="px-4 md:px-6 py-4 text-xs font-mono font-bold text-primary">{rental.id}</td>
                      ...
                      <td className="px-4 md:px-6 py-4 text-sm font-black text-right font-mono">{rental.total.toLocaleString()} RWF</td>
                      <td className="px-4 md:px-6 py-4 text-sm font-black text-right font-mono text-emerald-600 hidden md:table-cell">
                      ...
                      <td className="px-4 md:px-6 py-4 text-right">
                        <Link to={`/rentals/${rental.id}`} className="text-primary text-[10px] font-black uppercase tracking-widest hover:bg-primary/10 px-3 py-1.5 rounded-lg border border-primary/20 transition-all inline-block">Manage</Link>
                      </td>
                    </tr>
                  );
                })

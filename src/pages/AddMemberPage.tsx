...
const AddMemberPage: React.FC = () => {
  ...
  const [role, setRole] = useState<'Admin' | 'Staff' | 'Inspector'>('Staff');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newUser: User = {
      ...
      ip: '0.0.0.0',
      avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=1152d4&color=fff`
    };
    addUser(newUser);
        ...
        <form className="p-8 space-y-6" onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex flex-col gap-2">
              ...
              <input value={name} onChange={e => setName(e.target.value)} className="w-full px-4 py-3 rounded-lg border-[#dbdfe6] focus:ring-primary" placeholder="e.g. John Doe" required />
            </div>
            <div className="flex flex-col gap-2">
              <label className="text-[#111318] text-sm font-semibold">Work Email</label>
              <input value={email} onChange={e => setEmail(e.target.value)} className="w-full px-4 py-3 rounded-lg border-[#dbdfe6] focus:ring-primary" type="email" placeholder="john@company.com" required />
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2 pt-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {(['Staff', 'Inspector', 'Admin'] as const).map((r) => (
                <label key={r} className="relative cursor-pointer group">
                  <input className="peer hidden" name="role" type="radio" value={r} checked={role === r} onChange={() => setRole(r)} />
                  <div className="p-4 rounded-xl border-2 border-[#f0f2f4] transition-all peer-checked:border-primary peer-checked:bg-primary/5 hover:border-primary/50">
                    ...
                    <p className="text-xs text-gray-500 mt-1">{r === 'Admin' ? 'Full system access' : 'Standard access'}</p>
                  </div>
                </label>
              ))}
            </div>

  ...
  return (
    <div className="p-8 max-w-[1400px] mx-auto">
      <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Damage Assessment Dashboard</h2>
          <p className="text-slate-500 dark:text-slate-400 mt-1 font-medium">Refined tracking for reported incidents and asset restoration workflow.</p>
        </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-slate-200 dark:border-gray-800 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100 dark:border-gray-800 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="flex gap-2 p-1 bg-slate-100 dark:bg-gray-800 rounded-lg">
            {['All', 'Needs Review', 'Approved', 'Fixed', 'Sent to Insurance'].map((tab) => (
              <button 
                ...
                className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${activeStatus === tab ? 'bg-white dark:bg-gray-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500'}`}
              >{tab}</button>
            ))}
          </div>
          <select 
          ...
          <table className="w-full text-left">
            <thead className="bg-slate-50/50 dark:bg-gray-800/50">
              <tr>
                ...
                <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Valuation</th>
                ...
                filteredIncidents.map((i) => (
                  <tr key={i.id} className="hover:bg-slate-50/50 dark:hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <Link to={`/incidents/${i.id}/assess`} className="bg-rose-600 text-white text-[10px] font-black uppercase tracking-widest px-4 py-1.5 rounded-lg hover:bg-rose-700 shadow-sm transition-all">Assess</Link>
                    </td>
                  </tr>
                ))
              )}
